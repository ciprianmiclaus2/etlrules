...
SCENARIOS = [
    ...
    #    {"A": 3, "B": "b", "C": 5, "D": "e"},
    #]],
    [["B"], {"C": "sum", "D": "max", "E": "min", "F": "first"}, None, [
...
def test_aggregate_scenarios(group_by, aggregations, aggregation_expressions, expected, backend):
    ...
    with get_test_data(input_df, named_inputs={"input": input_df}, named_output="result") as data:
        rule = backend.rules.AggregateRule(group_by, aggregations=aggregations, aggregation_expressions=aggregation_expressions, named_input="input", named_output="result")
        rule.apply(data)
