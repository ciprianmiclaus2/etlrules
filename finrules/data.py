...
class RuleData:
    def __init__(self, main_input=None, named_inputs=None, strict=True):
        ...
        self.named_outputs = {name: df for name, df in named_inputs.items()} if named_inputs else {}
        self.lineage_info = {}
    ...
    def set_named_output(self, name, df):
        if self.strict:
            assert name not in self.named_outputs, f"{name} already exists as a named output. It will be overwritten."
